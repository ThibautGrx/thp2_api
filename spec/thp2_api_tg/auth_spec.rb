describe Thp2ApiTg::Auth, :vcr do
  let(:client) do
    Thp2ApiTg::Auth.new(nil, username: "tibog", password: "12345678", url: "http://0.0.0.0:3000")
  end

  it "is a Faraday middleware" do
    expect(Thp2ApiTg::Auth.ancestors).to include(Faraday::Middleware)
  end
  ...
  describe "#sign_in" do
    ...
    it "calls the api with the right params" do
      expect(Faraday).to receive(:post).with("http://0.0.0.0:3000/auth/sign_in", login: "tibog", password: "12345678").and_return(double("headers", headers: {}))
      subject
    end
